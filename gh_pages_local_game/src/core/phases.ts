...
function isAddEffect(effect: EffectConfig): boolean {
  return [
    ...
    "addQuestionBaseScore",
    "addQuestionScore",
    "addExamPostBonusByQuestionTriggerCount",
    "convertAccuracyOverflowToQuestionMultiplier",
    "addExamMultiplier",
    "addExamScore",
    "addExamPostBonus",
    "addNextExamScore"
  ].includes(effect.op) || (effect.op === "adjustCurrentTotalScore" && effect.mode === "add");
}
...
function isMultiplyEffect(effect: EffectConfig): boolean {
  return [
    ...
    "destroyAllOtherAndMultiplyCurrentTotal"
  ].includes(effect.op) || (effect.op === "adjustCurrentTotalScore" && effect.mode === "multiply");
}
